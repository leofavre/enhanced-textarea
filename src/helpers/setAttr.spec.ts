...
describe('setAttr', () => {
  ...
  it('Calls setAttribute with the attribute name and an empty string if the value is true', () => {
    setAttr(element, 'height', true);
    expect(element.setAttribute).toHaveBeenCalledWith('height', '');
  });

  it('Calls removeAttribute with the attribute name if the value is null', () => {
    setAttr(element, 'height', null);
    expect(element.removeAttribute).toHaveBeenCalledWith('height');
  });

  it('Calls removeAttribute with the attribute name if the value is undefined', () => {
    setAttr(element, 'height');
    expect(element.removeAttribute).toHaveBeenCalledWith('height');
  });

  it('Calls removeAttribute with the attribute name if the value is false', () => {
